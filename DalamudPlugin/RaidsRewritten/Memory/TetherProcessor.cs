// Adapted from https://github.com/PunishXIV/Splatoon/blob/main/Splatoon/Memory/TetherProcessor.cs
// 5f9f5a6
using System;
...
public unsafe sealed class TetherProcessor(ILogger logger) : IDisposable
{
    ...
    private Action<uint, uint, uint, uint, uint>? onTetherCreateCallback;
    private Action<uint, uint, uint, uint>? onTetherRemovalCallback;

    public void Init(Action<uint, uint, uint, uint, uint> onTetherCreateCallback, Action<uint, uint, uint, uint> onTetherRemovalCallback)
    {
        this.onTetherCreateCallback = onTetherCreateCallback;
        this.onTetherRemovalCallback = onTetherRemovalCallback;
    ...
    private long ProcessTetherDetour(VfxContainer* a1, byte a2, ushort a3, long targetOID, byte a5)
    {
        ...
        try
        {
            ...
            {
                logger.Warn($"Tether owner object is null");
            }
            if(a1 != null && a1->OwnerObject != null)
            {
                if(targetOID == 0xE0000000)
                {
                    ...
                    onTetherRemovalCallback?.Invoke(a1->OwnerObject->EntityId, a2, a3, a5);
                ...
                else
                {
                    ...
                    onTetherCreateCallback?.Invoke(a1->OwnerObject->EntityId, (uint)targetOID, a2, a3, a5);
