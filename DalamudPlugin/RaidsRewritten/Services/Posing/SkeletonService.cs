...
namespace RaidsRewritten.Services.Posing;

public unsafe sealed class SkeletonService : IDisposable
{
    ...
    private readonly ObjectMonitorService monitorService;
    ...
    private readonly Dictionary<Skeleton, CharacterSkeletonContainer> _skeletonToCharacterSkeleton = [];
    ...
    public SkeletonService(DalamudServices dalamud, ObjectMonitorService monitorService, ILogger logger)
    {
        ...
        var hooking = dalamud.GameInteropProvider;
        ...
        monitorService.CharacterBaseMaterialsUpdated += OnCharacterBaseMaterialsUpdate;
        monitorService.CharacterBaseDestroyed += OnCharacterBaseCleanup;

        RefreshSkeletonCache();
    }

    public unsafe void RegisterForFrameUpdate(Skeleton? skeleton, CharacterSkeletonContainer characterSkeleton)
    {
        if(skeleton != null)
        {
            _skeletonToCharacterSkeleton[skeleton] = characterSkeleton;
            logger.Info("Registering skeleton {0}", _skeletonToCharacterSkeleton.Count);
        }
    }
    ...
    private void ApplyBrioTransforms(Skeleton skeleton, CharacterSkeletonContainer characterSkeleton)
    {
        for(int partialIdx = 0; partialIdx < skeleton.Partials.Count; ++partialIdx)
        {
            ...
            if(pose != null)
            {
                ...
                for(int boneIdx = 0; boneIdx < boneLength; ++boneIdx)
                {
                    ...
                    var bonePoseInfo = characterSkeleton.GetBonePose(bone);
                    ...
                    // Transitive actions
                    characterSkeleton.ExecuteTransitiveActions(bone, bonePoseInfo);
    ...
    private void BeginSkeletonUpdate()
    {
        ...
        foreach (var skeleton in _skeletons)
        {
            ...
            //    continue;
            if (_skeletonToCharacterSkeleton.ContainsKey(skeleton) is false)
                continue;
        ...
        foreach (var skeleton in _skeletonsToUpdate)
        {
            ApplyBrioTransforms(skeleton, _skeletonToCharacterSkeleton[skeleton]);
    ...
    private void RefreshSkeletonCache()
    {
        ...
        this.logger.Debug("Refreshing skeleton cache...");
        //_skeletonToPosingCapability.Clear();
        _skeletonToCharacterSkeleton.Clear();
        _skeletons.Clear();
        this.dalamud.Framework.Run(() =>
        {
            foreach (var actor in this.dalamud.ObjectTable)
            {
            ...
            this.logger.Debug("Skeleton cache refreshed");
        });
    ...
    private void ClearSkeleton(Skeleton skeleton)
    {
        ...
        _skeletonToCharacterSkeleton.Remove(skeleton);
        skeleton.Dispose();
    }
    ...
    private void OnCharacterBaseMaterialsUpdate(BrioCharacterBase* charaBase)
    {
        ...
        catch (Exception e)
        {
            ...
            this.logger.Error("Error during skeleton caching: {0}", e.ToStringFull());
        }
    }
    ...
    private void OnCharacterBaseCleanup(BrioCharacterBase* charaBase)
    {
        ...
        catch (Exception e)
        {
            ...
            this.logger.Error("Error during skeleton cleanup: {0}", e.ToStringFull());
        }
    }
    ...
    private nint UpdateBonePhysicsDetour(nint a1)
    {
        ...
        catch(Exception e)
        {
            ...
            this.logger.Error("Error during skeleton update: {0}", e.ToStringFull());
        }
        return result;
    }
    ...
    private void FinalizeSkeletonsHook(nint a1)
    {
        ...
        catch(Exception e)
        {
            ...
            this.logger.Error("Error during skeleton finalization: {0}", e.ToStringFull());
        }
    }
    ...
    public void Dispose()
    {
        ...
        this.monitorService.CharacterBaseMaterialsUpdated -= OnCharacterBaseMaterialsUpdate;
        this.monitorService.CharacterBaseDestroyed -= OnCharacterBaseCleanup;
    }
}
